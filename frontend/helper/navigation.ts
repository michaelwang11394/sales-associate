export async function getMostRecentEvent(window: any, store: string): Promise<any | null> {
  try {
    const localEventsRaw = await window.sessionStorage.getItem("webPixelShopifyLatestEvents" + store);
    if (!localEventsRaw) {
        return null; // Return null if there are no events stored
    }
